    ...
    <div class="bg-paper border-b border-stone-200">
        <div class="max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between flex-wrap gap-3">
            ...
            @if ($machines->isNotEmpty())
                <form wire:submit="searchEverywhere" class="flex items-center gap-1.5 px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg w-full sm:w-[26rem]" wire:loading.class="opacity-60" wire:target="searchEverywhere">
                    ...
                    <input
                        ...
                        class="flex-1 text-sm border-0 focus:ring-0 p-0 bg-transparent placeholder:text-stone-400"
                    >
                    @if ($globalSearched)
                        <button type="button" wire:click="clearGlobalSearch" class="text-xs text-stone-400 hover:text-stone-600">Clear</button>
                    @endif
                </form>
            @endif
